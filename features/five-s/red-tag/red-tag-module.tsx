...
import { approveAndCloseRedTag, assignRedTagAction, returnRedTagForRework, saveRedTagClosureEvidence, startRedTagAction, submitRedTagForReview } from "./store";
...
const STATUS_TONE: Record<RedTagStatus, "danger" | "warning" | "success" | "secondary"> = {
  Open: "danger", Assigned: "warning", "In Progress": "warning", "Awaiting Review": "success", "Rework Required": "danger", Closed: "secondary",
};
...
function Summary({ tags }: { tags: RedTag[] }) {
  const items = ["Total Tags", "Open", "In Progress", "Awaiting Review"].map((label) => ({
  ...
  return <PageContainer className="max-w-none">
    ...
    <section className="overflow-hidden rounded-xl border bg-card shadow-sm">
      <div className="grid gap-2 border-b bg-muted/15 p-3 md:grid-cols-3 xl:grid-cols-[minmax(220px,1.5fr)_repeat(5,minmax(130px,1fr))]">
        ...
        <Filter value={status} onChange={setStatus} label="All statuses" options={["Open", "Assigned", "In Progress", "Awaiting Review", "Rework Required", "Closed"]} />
      ...
      <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[1180px] text-sm"><thead className="border-b bg-muted/20 text-left text-[11px] uppercase tracking-wide text-muted-foreground"><tr>
        ...
        <td className="px-4 py-3 whitespace-nowrap">{tag.dueDate ? displayDate(`${tag.dueDate}T00:00:00`) : "Not set"}</td><td className="px-4 py-3"><Badge variant={STATUS_TONE[tag.status]}>{tag.status}</Badge></td>
...
export function RedTagDetailPage({ tagId }: { tagId: string }) {
  ...
  const canPlan = isLeader && tag.status !== "Closed" && tag.status !== "Awaiting Review"; const canWork = isMember && ["Assigned", "In Progress", "Rework Required"].includes(tag.status);
  ...
  return <PageContainer className="max-w-none"><FiveSPageHeader eyebrow="Red Tags / Details" title={tag.tagNumber} description={`${tag.itemName} · ${tag.section}`}
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1.7fr)_minmax(320px,.7fr)]"><div className="grid gap-5">
      <Card><CardContent className="grid gap-5 p-5 sm:grid-cols-2 lg:grid-cols-4"><Meta icon={Package} label="Item / Equipment" value={tag.itemName} /><Meta icon={Flag} label="Status" value={tag.status} /><Meta label="Zone / Section" value={`${tag.zone} · ${tag.section}`} /><Meta label="Reason" value={tag.reason === "Others" ? tag.customReason ?? tag.reason : tag.reason} /><Meta icon={UserRound} label="Responsible" value={tag.responsiblePersonName || "Unassigned"} /><Meta label="Priority" value={tag.priority ?? "Not set"} /><Meta icon={CalendarDays} label="Due Date" value={tag.dueDate ? displayDate(`${tag.dueDate}T00:00:00`) : "Not set"} /><Meta label="Raised By" value={`${tag.createdByName} · ${displayDate(tag.createdAt)}`} /></CardContent></Card>
      ...
      {canPlan && <Card><CardContent className="grid gap-4 p-5"><div><h2 className="font-semibold">Action Plan</h2><p className="mt-1 text-sm text-muted-foreground">As Zone Leader, define the work and assign it to a member of {tag.zone}.</p></div><Field label="Action Plan *"><Textarea value={plan} onChange={(e) => setPlan(e.target.value)} placeholder={tag.actionPlan || tag.requiredAction || "Describe the work required to clear this Red Tag"} /></Field><div className="grid gap-4 md:grid-cols-3"><Field label="Responsible Zone Member *"><Select value={memberId || tag.responsiblePersonId} onValueChange={(v) => setMemberId(v ?? "")}><SelectTrigger><SelectValue placeholder="Select member" /></SelectTrigger><SelectContent>{(zoneConfig?.members ?? []).map((member) => <SelectItem key={member.id} value={member.id}>{member.name}</SelectItem>)}</SelectContent></Select></Field><Field label="Due Date *"><Input type="date" value={dueDate || tag.dueDate || tag.targetDate} onChange={(e) => setDueDate(e.target.value)} /></Field><Field label="Priority *"><Select value={tag.priority && priority === "Medium" ? tag.priority : priority} onValueChange={(v) => setPriority((v ?? "Medium") as RedTagPriority)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{["Low", "Medium", "High", "Critical"].map((value) => <SelectItem key={value} value={value}>{value}</SelectItem>)}</SelectContent></Select></Field></div><Field label="Remarks / Instructions"><Textarea value={instructions} onChange={(e) => setInstructions(e.target.value)} placeholder={tag.instructions || "Optional instructions"} /></Field><div className="flex justify-end"><Button onClick={savePlan}>Save & Assign Action</Button></div></CardContent></Card>}
      ...
      {canWork && <Card><CardContent className="p-5"><h2 className="font-semibold">Zone Member Work</h2><div className="mt-4 flex flex-wrap gap-2">{["Assigned", "Rework Required"].includes(tag.status) && <Button onClick={() => startRedTagAction(tag.id, user)}>Start Action</Button>}{["In Progress", "Rework Required"].includes(tag.status) && <Button variant="outline" onClick={() => setCaptureOpen(true)}><Camera className="size-4" /> Add Closure Evidence</Button>}</div>{["In Progress", "Rework Required"].includes(tag.status) && <div className="mt-4 grid gap-3"><Field label="Completion Comment *"><Textarea value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Describe the work completed" /></Field><div className="flex justify-end"><Button disabled={!tag.closureImageUrl || !comment.trim()} onClick={() => submitRedTagForReview(tag.id, user, comment)}>Submit for Review</Button></div></div>}</CardContent></Card>}
      {isLeader && tag.status === "Awaiting Review" && <Card><CardContent className="p-5"><h2 className="font-semibold">Zone Leader Review</h2><p className="mt-1 text-sm text-muted-foreground">Compare the evidence and review the member&apos;s completion comment.</p><div className="mt-4 flex flex-wrap justify-end gap-2"><Button variant="outline" onClick={() => setReworkOpen(true)}>Return for Rework</Button><Button onClick={() => approveAndCloseRedTag(tag.id, user)}><CheckCircle2 className="size-4" /> Approve & Close</Button></div></CardContent></Card>}
...
export function RedTagPrintPage({ tagId }: { tagId: string }) {
  ...
  return <PageContainer className="red-tag-print-page max-w-none"><style>{`@media print { @page { size: A4 portrait; margin: 10mm; } }`}</style><div className="print:hidden"><FiveSPageHeader eyebrow="Red Tags / Print Preview" title="Print Red Tag" description="Print this label and attach it to the tagged item."
    ...
    <div className="red-tag-print-surface motion-success-in grid w-full place-items-center gap-6 rounded-xl border bg-muted/25 p-4 sm:p-8"><ReportHeader title={tag.status === "Closed" ? "Red Tag Closure Report" : "Red Tag Report"} reportId={tag.tagNumber} subtitle={`${tag.itemName} · ${tag.zone}`} className="w-full max-w-[800px] rounded-lg" /><RedTagLabel tag={tag} /></div>
  </PageContainer>;
}
