...
export function ZonePerformanceChart({ data }: { data: ZoneScorePoint[] }) {
  return <ChartCard title="Average Audit Score by Zone" description="Average audit performance across workplace zones.">{data.length ? <ResponsiveContainer width="100%" height="100%"><BarChart data={data} margin={{ top:8,right:18,left:-14,bottom:4 }}><CartesianGrid vertical={false} stroke="var(--border)" strokeDasharray="3 3" /><XAxis dataKey="zone" tickLine={false} axisLine={false} tick={{fill:"var(--muted-foreground)",fontSize:11}} /><YAxis domain={[0,100]} tickFormatter={(v)=>`${v}%`} tickLine={false} axisLine={false} tick={{fill:"var(--muted-foreground)",fontSize:11}} /><Tooltip contentStyle={tooltipStyle} formatter={(v)=>[`${v}%`,"Average score"]} labelFormatter={(label,payload)=>`${label}${payload?.[0]?.payload?.leader ? ` · ${payload[0].payload.leader}` : ""}`} /><Bar dataKey="score" fill="var(--primary)" radius={[5,5,0,0]} maxBarSize={42} /></BarChart></ResponsiveContainer> : <Empty text="No zone scores available" />}</ChartCard>;
}

export function CorrectiveActionsChart({ data }: { data: ActionStatusPoint[] }) {
  return <ChartCard title="Non-Compliances by Zone and Status" description="Open corrective workload compared with closed actions.">{data.length ? <ResponsiveContainer width="100%" height="100%"><BarChart data={data} margin={{top:8,right:12,left:-20,bottom:4}}><CartesianGrid vertical={false} stroke="var(--border)" strokeDasharray="3 3" /><XAxis dataKey="zone" tickLine={false} axisLine={false} tick={{fill:"var(--muted-foreground)",fontSize:11}} /><YAxis allowDecimals={false} tickLine={false} axisLine={false} tick={{fill:"var(--muted-foreground)",fontSize:11}} /><Tooltip contentStyle={tooltipStyle} /><Bar dataKey="Closed" stackId="status" fill="var(--success)" /><Bar dataKey="Open" stackId="status" fill="var(--muted-foreground)" radius={[4,4,0,0]} /></BarChart></ResponsiveContainer> : <Empty text="No non-compliances during this period" />}</ChartCard>;
}

export function ImprovementsTrend({ data }: { data: TrendPoint[] }) {
  return <ChartCard title="Total Improvements by Period" description="Corrective actions successfully completed over time.">{data.length ? <ResponsiveContainer width="100%" height="100%"><LineChart data={data} margin={{top:8,right:18,left:-22,bottom:4}}><CartesianGrid vertical={false} stroke="var(--border)" strokeDasharray="3 3" /><XAxis dataKey="label" tickLine={false} axisLine={false} tick={{fill:"var(--muted-foreground)",fontSize:11}} /><YAxis allowDecimals={false} tickLine={false} axisLine={false} tick={{fill:"var(--muted-foreground)",fontSize:11}} /><Tooltip contentStyle={tooltipStyle} formatter={(v)=>[v,"Completed improvements"]} /><Line type="monotone" dataKey="value" stroke="var(--primary)" strokeWidth={2.5} dot={{r:3,fill:"var(--card)",strokeWidth:2}} activeDot={{r:5}} /></LineChart></ResponsiveContainer> : <Empty text="No completed improvements during this period" />}</ChartCard>;
}
