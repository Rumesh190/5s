...
export default function FiveSReferenceGuide({ question, questionText }: { question: FiveSQuestion; questionText: string }) {
  ...
  return <>
    <Tooltip><TooltipTrigger render={<Button type="button" variant="ghost" size="icon" className="size-11 shrink-0 text-muted-foreground hover:bg-primary/8 hover:text-primary focus-visible:ring-2 focus-visible:ring-ring md:size-8" aria-label={`View reference for: ${questionText}`} onPointerDown={(event) => event.stopPropagation()} onClick={(event) => { event.stopPropagation(); setOpen(true); }} />}><Info className="size-4" aria-hidden="true" /></TooltipTrigger><TooltipContent>{t("audit.viewReference")}</TooltipContent></Tooltip>
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent showCloseButton={false} className="grid max-h-[calc(100dvh-24px)] !w-[calc(100vw-24px)] !max-w-none min-w-0 grid-rows-[auto_minmax(0,1fr)_auto] gap-0 overflow-hidden rounded-[20px] p-0 sm:max-h-[calc(100dvh-40px)] sm:!w-[calc(100vw-48px)] sm:!max-w-none sm:min-w-0 lg:max-h-[calc(100dvh-64px)] lg:!w-[calc(100vw-96px)] lg:!max-w-[960px] lg:min-w-[760px] lg:rounded-[24px]">
        <header className="relative border-b px-5 py-4 pr-16 sm:px-8 lg:px-10"><DialogTitle>{t("audit.referenceGuide")}</DialogTitle><Button type="button" variant="ghost" size="icon" className="absolute right-3 top-1.5 size-11 sm:right-5 sm:top-2.5 sm:size-9 lg:right-7" aria-label={t("common.close")} onClick={() => setOpen(false)}><X className="size-4" /></Button></header>
