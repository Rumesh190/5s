...
export default function FinalAuditVerificationDialog({
  ...
  onComplete: (capture: AuditVerificationCapture) => boolean | void;
}) {
  ...
  async function startCamera() {
    ...
    try {
      ...
      setCameraError(denied ? "Camera access is required to take your auditor photo. Check your browser permissions and try again." : unavailable ? "Camera unavailable. No camera was detected on this device." : "Unable to access the camera. Check that it is not already in use and try again.");
      stopCamera();
    } finally {
      startInFlightRef.current = false;
  ...
  function capturePhoto() {
    ...
    setPhoto(normalizedEvidenceDataUrl(canvas, 0.8));
    stopCamera();
  ...
  function complete() {
    ...
    setCompleting(true);
    stopCamera();
    const completed = onComplete({ photo, signature, capturedAt: new Date().toISOString() });
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-4 sm:px-6 lg:px-5">
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <section ref={signatureSectionRef} className="flex h-full min-w-0 scroll-mt-4 flex-col rounded-xl border p-4">
            <StepHeader step="2" complete={Boolean(signature)} title="Signature" description="Provide your signature to complete this audit" required />
