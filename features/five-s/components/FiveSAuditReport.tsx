    ...
    <div className="audit-report-page -m-5 min-h-screen bg-slate-100 p-4 text-slate-950 dark:bg-slate-950 dark:text-slate-100 sm:-m-6 sm:p-6 lg:-m-7 xl:-m-8">
      ...
      <article className="audit-report-document mx-auto hidden min-w-0 max-w-[1120px] space-y-5 overflow-hidden rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:space-y-7 sm:p-8 md:block print:block">
        ...
        <ReportSection title={t("reports.correctiveActions")}>
          ...
          {actions.length ? <div className="audit-actions-grid grid gap-3 md:grid-cols-2">{actions.map((action) => {
            ...
            return <article key={action.id} className="audit-report-block rounded-lg border border-slate-200 p-4 dark:border-slate-700">
              ...
              <dl className="mt-3 grid grid-cols-2 gap-2 text-xs">
                <div><dt className="text-muted-foreground">Category</dt><dd className="font-medium">{getActionCategoryDisplay(action, action.category ?? "—")}</dd></div>
          ...
          })}</div> : <p className="text-sm text-muted-foreground">No Actions were created for this audit.</p>}
        </ReportSection>
